...
app.get('/inventory/:playerId', async (req, res) => {
    ...
    {
        res.status(500).json({ seuccess: false, message : error.message});
    }
});
...
app.get('/quests/:status', async (req,res) => {
    ...
    {
        await pool.query(
            `UPDATE player_quests SET status = ?, complete - IF(? = "완료", CURRENT_TIMESTAMP, null) WHERE player_id = ? AND quest_id = ?`,
